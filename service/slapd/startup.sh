  ...
  if [ "${KEEP_EXISTING_CONFIG,,}" == "true" ]; then
    ...
    if [ "${LDAP_REPLICATION,,}" == "true" ]; then
      ...
      do
        ...
        sed -i "s|{{ LDAP_REPLICATION_HOSTS_CONFIG_SYNC_REPL }}|olcSyncRepl: rid=00$i provider=${!host} ${LDAP_REPLICATION_CONFIG_SYNCPROV_MASTER}\n{{ LDAP_REPLICATION_HOSTS_CONFIG_SYNC_REPL }}|g" ${CONTAINER_SERVICE_DIR}/slapd/assets/config/replication/replication-enable.ldif
        if [ -n "${LDAP_REPLICATION_CONFIG_2_SYNCPROV_MASTER}" ]; then
          sed -i "s|{{ LDAP_REPLICATION_HOSTS_CONFIG_SYNC_REPL }}|olcSyncRepl: rid=20$i provider=${!host} ${LDAP_REPLICATION_CONFIG_2_SYNCPROV_MASTER}\n{{ LDAP_REPLICATION_HOSTS_CONFIG_SYNC_REPL }}|g" ${CONTAINER_SERVICE_DIR}/slapd/assets/config/replication/replication-enable.ldif
        fi
        sed -i "s|{{ LDAP_REPLICATION_HOSTS_DB_SYNC_REPL }}|olcSyncRepl: rid=10$i provider=${!host} ${LDAP_REPLICATION_DB_SYNCPROV_MASTER}\n{{ LDAP_REPLICATION_HOSTS_DB_SYNC_REPL }}|g" ${CONTAINER_SERVICE_DIR}/slapd/assets/config/replication/replication-enable.ldif
      ...
      if [ "${READ_ONLY_REPLICA,,}" == "true" ]; then
        sed -i "/{{ MIRROR_MODE }}/d" ${CONTAINER_SERVICE_DIR}/slapd/assets/config/replication/replication-enable.ldif
      else
        sed -i "s|{{ MIRROR_MODE }}|-\nadd: olcMirrorMode\nolcMirrorMode: TRUE|g" ${CONTAINER_SERVICE_DIR}/slapd/assets/config/replication/replication-enable.ldif
      fi
